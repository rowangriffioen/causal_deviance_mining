...
def predictive_measuring_patterns(patterns_data, patient_data, label_class, outcome_type):
    if outcome_type == 'binary':
        # NEW: handle the "no patterns" case
        if patterns_data.empty or 'patterns' not in patterns_data or len(patterns_data['patterns']) == 0:
            patterns_data['Outcome_Interest'] = pd.Series(dtype=float)
            return patterns_data

        x = patient_data[patterns_data['patterns']].fillna(0)
        # If x ends up with zero columns, return empty scores
        if x.shape[1] == 0:
            patterns_data['Outcome_Interest'] = pd.Series(dtype=float)
            return patterns_data

        cat_y_all = patient_data[label_class]
        # ensure discrete integer dtype for MI on discrete features
        x = x.astype(int)
        cat_y_all = pd.Series(cat_y_all).astype(int)
...
def create_pattern_attributes(patient_data, label_class, pattern_list,
    ...
    # NEW: handle empty list early
    if not pattern_list:
        cols = ['patterns', 'Pattern_Frequency', 'Case_Support',
...
def plot_dashboard(fig, ax, patient_data, numerical_attributes, categorical_attributes, tab_name):
    ...
    for cat in categorical_attributes:
        ...
        ax[r, jj].pie(
            pd.DataFrame(
                patient_data.loc[patient_data[tab_name] > 0, cat].value_counts()).sort_index()['count'],
            radius=1 - 0.4,
...
def Single_Pattern_Extender(all_extension_list, chosen_pattern_ID,
    ...
    for idx, case in enumerate(all_extension_list[chosen_pattern_ID]['Instances']['case']):
        ...
        if Eventual_preceding and len(in_pattern_nodes) > 0:
            Eventual_preceding_nodes = {node for node in Eventual_relations_nodes if
                                        min(in_pattern_nodes) - Max_gap_between_events <= node < min(in_pattern_nodes)}
            for Ev_P_nodes in Eventual_preceding_nodes:
