...
from helper_functions.feature_extraction.deviancecommon import read_XES_log, split_log_train_test
...
from helper_functions.feature_extraction.TraceUtils import trace_to_label_map
...
def payload_embedding(inp_folder, settings_file, log, _unused, self = None):
    print(f"Lengths of log: {len(log)}")
    pex = PayloadExtractor()
    settings = settings_from_cfg(settings_file)
    extracted_data = pex.get_overview_of_data(log, settings)
    df, _ = build_dataframes(extracted_data, extracted_data, settings)
    df = extendDataFrameWithLabels(df, trace_to_label_map(log))
    # make an empty “test” DataFrame with the same columns so genericDump’s quality checks pass
    test_df = df.iloc[0:0]
    j = genericDump(inp_folder, df, test_df, "payload.csv", None)
    if self is not None:
        dump_in_primary_memory_as_table_csv(self, "payload_for_training", df, test_df)
    # return the set of Case_IDs that we just wrote
    return set(df["Case_ID"]) if "Case_ID" in df.columns else set()
