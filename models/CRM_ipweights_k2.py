...
def create_fair_dataset_ipweights(dataset, LHS_set:list, RHS, C:list):
    """
    ...
    for name, df_c in df_C:
        ...
        mask_p =1
        for i in LHS_set:
            mask_p = mask_p & (df_c[i] == 1)
        # mask_p = df_C [ LHS_set ] . all ( axis=1)
        df_p = df_c [all_relevant_vars][ mask_p ]
        df_not_p = df_c [all_relevant_vars][ ~mask_p ]
        len_df_p=len(df_p)
        len_df_not_p=len(df_not_p)
        df_p = df_c[all_relevant_vars][mask_p]
        df_not_p = df_c[all_relevant_vars][~mask_p]
        ...
        if count==0:
            continue
        df_p_ratio=df_p_count/df_c_count
        df_not_p_ratio=df_not_p_count/df_c_count
        weight_p=1/df_p_ratio
        weight_not_p=1/df_not_p_ratio

        pRHS_count=df_p[RHS].sum()
        pnotRHS_count = df_p_count - pRHS_count
        notpRHS_count=df_not_p[RHS].sum()
        notpnotRHS_count = df_not_p_count - notpRHS_count

        wpRHS_count=pRHS_count*weight_p
        wpnotRHS_count=pnotRHS_count*weight_p
        wnotpRHS_count=notpRHS_count*weight_not_p
        wnotpnotRHS_count=notpnotRHS_count*weight_not_p

        fair_dataset= fair_dataset._append(df_p, ignore_index=True)

        fair_dataset= fair_dataset._append(df_not_p, ignore_index=True) 
        ...
        if (wpRHS_count <= wnotpnotRHS_count):
            n12+=wpRHS_count               # there are at most pRHS_count possible matched pairs containing RHS in exposure group (p) and not RHS in non-exposure group (not p)
            n21+=wnotpRHS_count            # there are at most notpRHS_count possible matched pairs containing not RHS in exposure group (p) and RHS in non-exposure group (not p)
        else:
            n12+=wnotpnotRHS_count         # there are at most notpnotRHS_count possible matched pairs containing RHS in exposure group (p) and not RHS in non-exposure group (not p)
            n21+=wpnotRHS_count            # there are at most pnotRHS_count possible matched pairs containing not RHS in exposure group (p) and RHS in non-exposure group (not p)

    return len(fair_dataset.index), n12, n21, fair_dataset
...
def _evaluate_LHS(args):
    ...
    count, n12, n21, df_fair = create_fair_dataset_ipweights(data, LHS, RHS, C, RATIO)
    if n21 == 0:
        n21 = 1
    if n12 == 0:
        n12 = 1
