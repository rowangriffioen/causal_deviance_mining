...
def create_fair_dataset_ipweights(dataset, LHS_set: list, RHS, C: list):
    ...
    # >>> added: robust grouping when C is empty
    if not C:
        groups = [(None, dataset[all_relevant_vars])]
    else:
        groups = dataset[all_relevant_vars].groupby(C)
    # <<<

    for name, df_c in groups:
...
def CRCS(data, variables, RHS, min_l_supp, max_k):
    ...
    #13
            count, n12, n21, df_fair = create_fair_dataset_ipweights(data, LHS, RHS, C)
...
def printrules(pRc,nRc,cvl,data):
    df_rules = pd.DataFrame(columns = ['Rule', 'Odds ratio', 'LB odds ratio',  'Support LHS','Support RHS','Confidence','Lift','Conviction', 'n12','n21', 'Fair set count'])
    for r in pRc:
        ...
        df_rules = df_rules._append({'Rule':rule,'Odds ratio': rules_or[tuple(r)],'LB odds ratio': rules_lbor[tuple(r)],'Support LHS':suppLHS,'Support RHS':suppRHS,'Confidence':c, 'Lift': l,'Conviction':conv , 'n12': rules_n12[tuple(r)], 'n21': rules_n21[tuple(r)],'Fair set count': rules_fairsetcount[tuple(r)] },ignore_index = True)
    for r in nRc:
        suppLHS,suppRHS,c,l,conv=computeMetrics(r,'notZ',data)
        rule= str(r)+" --> !" + target
        b=iscontrollable(r,cvl)
        df_rules = df_rules._append({'Rule':rule,'Odds ratio': rules_or[tuple(r)],'LB odds ratio': rules_lbor[tuple(r)],'Support LHS':suppLHS,'Support RHS':suppRHS, 'Confidence':c, 'Lift': l, 'Conviction':conv ,'n12': rules_n12[tuple(r)], 'n21': rules_n21[tuple(r)],'Fair set count': rules_fairsetcount[tuple(r)]},ignore_index = True)
    df_rules.to_csv("rules_"+name+"_"+target+".csv",sep=";",index=False)
