...
def main():
    ...
    config_path = os.path.join("config", "config_dm_crm_k1.yaml")
    ...
    for enc in encodings:
        ...
        out_dir = os.path.join("4_output", "ipweights", event_log, f"{experiment}_features", "k1", enc)
        ...
        tmp_cfg = f"temp_crm_ipweights_{experiment}_{enc}_k1.yaml"
        with open(tmp_cfg, "w") as tf:
            yaml.dump(crm_cfg, tf)

        # Execute discovery script; on success move artifacts; always clean temp config
        try:
            subprocess.run(["python", "models/CRM_ipweights_k1.py", tmp_cfg], check=True)

            generated = f"rules_ipweights_{enc}_k1_{target_var}.csv"
            timing    = f"seconds_runtime_ipweights_{experiment}_{enc}_k1.txt"
