...
def main():
    ...
    config_path = BASE_DIR / "config" / "config_dm_crm_k2.yaml"
    ...
    for enc in encodings:
        print(f"\n=== CRM k2 on encoding: {enc} ===")

        # Paths
        dataset_file_abs = (base_input / enc / f"{enc}.csv").resolve()
        out_dir = BASE_DIR / "4_output" / "random" / event_log / f"{experiment}_features" / "k2" / enc
        ...
        # Put a RELATIVE path in YAML (avoids './C:\\...' issues in the called script)
        try:
            dataset_file_rel = dataset_file_abs.relative_to(BASE_DIR)
        except ValueError:
            dataset_file_rel = dataset_file_abs  # fallback if outside BASE_DIR (unlikely)
        ...
        tmp_cfg = BASE_DIR / f"temp_crm_random_{experiment}_{enc}_k2.yaml"
        ...
        try:
            subprocess.run(
                [sys.executable, "models/CRM_random_k2.py", tmp_cfg.name],
                check=True,
                cwd=BASE_DIR,  # important for relative paths inside the model script
            )

            generated = f"rules_random_{enc}_k2_{target_var}.csv"
            timing    = f"seconds_runtime_random_{experiment}_{enc}_k2.txt"
