...
def main():
    ...
    config_path = BASE_DIR / "config" / "config_dm_crm_k3.yaml"
    ...
    for enc in encodings:
        print(f"\n=== CRM k3 on encoding: {enc} ===")

        # Paths
        dataset_file_abs = (base_input / enc / f"{enc}.csv").resolve()
        out_dir = BASE_DIR / "4_output" / "random" / event_log / f"{experiment}_features" / "k3" / enc
        ...
        tmp_cfg = BASE_DIR / f"temp_crm_random_{experiment}_{enc}_k3.yaml"
        ...
        try:
            subprocess.run(
                [sys.executable, "models/CRM_random_k3.py", tmp_cfg.name],
                check=True,
                cwd=BASE_DIR,  # make relative paths in the model script resolve from project root
            )
